...
async function getAssignmentGroupMonitoring(assignmentId) {
  const assignRes = await pool.query('SELECT id, title FROM assignments WHERE id = $1', [assignmentId]);
  ...
  if (isAllStudents) {
    relevantGroupsRes = await pool.query('SELECT id, name, created_at FROM groups ORDER BY name ASC');
  } else {
...
async function getAssignmentStudentMonitoring(assignmentId) {
  const assignRes = await pool.query('SELECT id, title FROM assignments WHERE id = $1', [assignmentId]);
  if (assignRes.rows.length === 0) {
    const err = new Error('Assignment not found');
    err.statusCode = 404;
  ...
  if (isAllStudents) {
    ...
    studentsRes = await pool.query(`
      ...
      JOIN users u ON gm.student_id = u.id
      LEFT JOIN submission_confirmations sc ON sc.assignment_id = $1 AND sc.student_id = u.id
      WHERE at.assignment_id = $1 AND u.role = 'student'
...
async function getCompletionAnalytics(assignmentIdFilter = null) {
  ...
  for (const a of assignmentsRes.rows) {
    // Determine expected submissions for this assignment
    let expectedCount = 0;
    ...
    byAssignment.push({
      ...
      is_complete: expectedCount > 0 && confirmedCount === expectedCount
    });
  }

  const overallPercentage = totalExpected > 0 ? Math.round((totalConfirmed / totalExpected) * 100) : 0;
...
async function getGroupPerformanceAnalytics() {
  const groupsRes = await pool.query('SELECT id, name FROM groups ORDER BY name ASC');
  ...
  for (const group of groupsRes.rows) {
    ...
    const memberCount = membersCountRes.rows[0].count;

    // Assignments applicable to this group
    ...
    const assignmentsCount = assignmentsRes.rows.length;
    const expectedConfirmations = memberCount * assignmentsCount;

    // Confirmed submissions from members of this group for applicable assignments
    const confirmedRes = await pool.query(`
...
async function getDashboardSummary() {
  const [studentsRes, groupsRes, assignmentsRes, analytics] = await Promise.all([
  ...
  const recentSubmissionsRes = await pool.query(`
    SELECT sc.id, sc.confirmed_at, u.name AS student_name, u.email AS student_email,
           a.title AS assignment_title, COALESCE(g.name, 'No Group') AS group_name
    FROM submission_confirmations sc
